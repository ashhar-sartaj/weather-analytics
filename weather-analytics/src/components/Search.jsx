...
function Search({cities, onFetchAll, isLoading, forecasts, onRemoveCity}) {
    ...
    const errorInGeofetching = useSelector((state) => state.weather.geoError); //if error in geoFetching is not null.. means we our goe api promise is rejected..so ask the user to enter valid location inputs.
    ...
    //    console.log('city name:',cityName)
        ...
        // const zip = zip.trim();
        const locationKey = {
            id: `${zip.trim()}-${countryCode.trim()}`,
            zip:`${zip.trim()}`,
            country:`${countryCode.trim()}`
        } //this location will be sent as payload for getting geocoords..
        ...
        setZip('');
        try {
            const geoCoordResponse = await dispatch(geoCoordinatesForCity(locationKey)); //the return result of this api call will serve as the payload.
            ...
            console.log(geoCoord); //the result of above is then added to the cities state.
            const cityDetails = {
                id: geoCoord.id,
                zip: geoCoord.data.zip,
                country:geoCoord.data.country,
                name:geoCoord.data.name,
                lat: geoCoord.data.lat,
                lon:geoCoord.data.lon,
            }
            //aftre getting the geocoords, now add the city. with id: `${zipCode}-${countryCode}`, name:`${cityName}`,
            dispatch(addCity(cityDetails));
        } catch(error) {
            // thunk already stored error in Redux
            return;
        }
        // const cityDetails = {
        //     id: geoCoord.id,
        ...
        // //aftre getting the geocoords, now add the city. with id: `${zipCode}-${countryCode}`, name:`${cityName}`,
        // dispatch(addCity(cityDetails));
       //create an object that is sent as payload for weatherForecastForCity. This way we can provide the unit parameter to the api call.
       //first set the react state that changes the unit: to and fro-- we dispatch action
    ...
    //     unit: unit
    //    }
    //    const weatherForecastResponse = await dispatch(weatherForecastForCity(payloadForForecast));
...
return (
    ...
    <div className="search-container">
        <form className="search-container-child search-container-form">
            ...
            {/* showing error if the entered location details are invalid */}
            {errorInGeofetching && <p className="errorInGeoFetching">Please enter valid details or check internet connection.</p>}
        </form>
